...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{
		MessageBox(NULL, (LPCWSTR) "Could not open texture file", (LPCWSTR) "ERROR", MB_OK);	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		MessageBox(NULL, (LPCWSTR) "Could not read file header", (LPCWSTR) "ERROR", MB_OK);		// If it fails, display an error message 
	...
	{
		MessageBox(NULL, (LPCWSTR) "TGA file be type 2 or type 10 ", (LPCWSTR) "Invalid Image", MB_OK);	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{										
		MessageBox(NULL, (LPCWSTR) "Could not read info header", (LPCWSTR) "ERROR", MB_OK);		// Display error
	...
	{
		MessageBox(NULL, (LPCWSTR) "Invalid texture information", (LPCWSTR) "ERROR", MB_OK);	// Display Error
	...
	{
		MessageBox(NULL, (LPCWSTR) "Could not allocate memory for image", (LPCWSTR) "ERROR", MB_OK);	// Display Error
	...
	{
		MessageBox(NULL, (LPCWSTR) "Could not read image data", (LPCWSTR) "ERROR", MB_OK);		// Display Error
...
{ 
	...
	{
		MessageBox(NULL, (LPCWSTR) "Invalid texture information", (LPCWSTR) "ERROR", MB_OK);	// If it isnt...Display error
	...
	do
	{
		...
		{
			MessageBox(NULL, (LPCWSTR) "Could not read RLE header", (LPCWSTR) "ERROR", MB_OK);	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					MessageBox(NULL, (LPCWSTR) "Could not read image data", (LPCWSTR) "ERROR", MB_OK);		// IF we cant, display an error
				...
				{
					MessageBox(NULL, (LPCWSTR) "Too many pixels read", (LPCWSTR) "ERROR", NULL);			// if there is too many... Display an error!
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{	
				MessageBox(NULL, (LPCWSTR) "Could not read from file", (LPCWSTR) "ERROR", MB_OK);			// If attempt fails.. Display error (again)
